...
#Subset Greek
#crime_greek <- count(crime_nox, vars=c("IncidentGroups","Weekdays", "Month", "AREA_COLOR"))
...
ggsave(file = "weekday_theft_full.png", width = 10, height = 10)
...
final_allcrime <- ggplot(crime_all)+geom_bar(aes(x=Weekdays, y=freq), stat="identity", fill = "#01516c")+facet_wrap(~Incident.Type, nrow = 6)
...
#Save as .png.
#Height and width measured in cm? not pixels
ggsave(file = "weekday_allcrime_full.png", width = 10, height = 10)

##########################
#Frequency and Day of the Week, All Crime, Greek Locations
##########################

#Graph bar plot x= Weekday y= freq of all crime at locations K
#final_greek <- ggplot(crime_greek)+geom_bar(aes(x=Weekdays, y=freq), stat="identity", fill = "#01516c")+facet_grid(AREA_COLOR~IncidentGroups)

#Economist theme
#final_greek + theme_economist(stata = TRUE) + scale_fill_economist( stata = TRUE ) + labs( title = "Crimes at USC near the Row by Weekday \n", x= "Weekday", y = "Frequency") +
#  scale_y_continuous(breaks =seq(0,100,1)) +
#  theme(axis.text.x=element_text(size = 10, angle = -90, hjust = 1, vjust = 0.5), axis.title.x=element_text(size=12),
#        axis.title.y=element_text(size=12, vjust = 1.5), panel.margin = unit(1.5, "lines"), plot.title = element_text(size = 14,hjust = 0.5))
  #+geom_text(data= crime_greek_sum, aes(x=Weekdays, y=sums,label = crime_greek_sum$sums), vjust= 0, size =4)
#Save as .png.
#Height and width measured in cm? not pixels
#ggsave(file = "weekday_allcrime_greek_grid.png", width = 10, height = 10)
...
final_time <- ggplot(crime_time)+geom_bar(aes(x=TimeGroups, y=freq), stat="identity", fill = "#01516c")+facet_wrap(~IncidentGroups, ncol = 3)
...
#Save as .png.
#Height and width measured in cm? not pixels
ggsave(file = "weekday_timegroups_full.png", width = 10, height = 10)

##########################
#Frequency and Time Groupings by Locations
##########################

#Graph bar plot x= Time Groupings y= Frequency by Crime groups
#final_timelocation <- ggplot(crime_timelocation)+geom_bar(aes(x=TIME_GROUP, y=freq), stat="identity", fill = "#01516c")+facet_grid(AREA_COLOR~IncidentGroups)
#ggsave(file = "weekday_timelocation.png", width = 10, height = 10)
#Economist theme
#final_timelocation + theme_economist(stata = TRUE) + scale_fill_economist( stata = TRUE ) + labs( title = "Crimes at USC by Location \n", x= "Time Groups", y = "Frequency") +
#  scale_y_continuous(breaks =seq(0,100,15)) +
#  theme(axis.text.x=element_text(size = 10, angle = -90, hjust = 1, vjust = 0.5), axis.title.x=element_text(size=12),
#        axis.title.y=element_text(size=12, vjust = 1.5), plot.title = element_text(size = 14,hjust = 0.5))

#Save as .png.
#Height and width measured in cm? not pixels
#ggsave(file = "weekday_timelocation_full.png", width = 10, height = 10)
...
#Graph bar plot x= Time Groupings y= Frequency by Crime groups
#final_timelocation <- ggplot(crime_timelocation)+geom_point(aes(x=TIME_GROUP, y=freq), color = "#01516c")+facet_grid(AREA_COLOR~IncidentGroups)
...
#Save as .png.
#Height and width measured in cm? not pixels
#ggsave(file = "weekday_timelocation_dot.png", width = 10, height = 10)